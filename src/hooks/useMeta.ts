...
const useMeta = ({
  ...
  const originalKeywords = React.useRef(
    documentDefined
      ? document.querySelector('meta[name="keywords"]')?.getAttribute('content')
      : null,
  );
  const originalAuthor = React.useRef(
    documentDefined
  ...
  React.useEffect(() => {
    ...
    if (description && document.querySelector('meta[name="description"]')) {
      ...
      document
        .querySelector('meta[name="description"]')!
        .setAttribute('content', originalDescriptionValue);
    }
    if (keywords && document.querySelector('meta[name="keywords"]')) {
      ...
      document
        .querySelector('meta[name="keywords"]')!
        .setAttribute('content', originalKeywords.current);
    }
    if (authors && document.querySelector('meta[name="author"]')) {
      document
        ...
        .setAttribute(
          'content',
          `Game built by Andy Challis, designed by ${authors.join(', ')}`,
        );
    } else if (originalAuthor.current) {
      document
        .querySelector('meta[name="author"]')!
        .setAttribute('content', originalAuthor.current);
    }
